...
function openImgLightbox(url, prompt, style) {
  ...
  dl.href = url; dl.download = 'nexus-image.png';
  const ep = document.getElementById('img-edit-panel');
...
function toggleImgEditPanel() {
  const ep = document.getElementById('img-edit-panel');
  if (!ep) return;
  ep.style.display = ep.style.display === 'none' ? 'block' : 'none';
}
async function applyImgEdit(action) {
  const imgEl  = document.getElementById('img-lightbox-img');
  const status = document.getElementById('img-edit-status');
  const dl     = document.getElementById('img-lightbox-dl');
  if (!imgEl || !_lightboxCurrentUrl) return;
  if (status) { status.style.display = 'block'; status.textContent = 'Applying…'; }

  let imgPayload = {};
  const src = _lightboxCurrentUrl;
  if (src.startsWith('data:')) {
    imgPayload = { image: src };
  } else {
    // Fetch external URL via proxy to avoid CORS, then base64 it
    try {
      const res = await fetch('/api/proxy-image', { method:'POST', headers:{'Content-Type':'application/json'},
        body: JSON.stringify({ url: src }) });
      if (res.ok) {
        const d = await res.json();
        imgPayload = { image: d.b64 || src, url: d.b64 ? undefined : src };
      } else {
        imgPayload = { url: src };
      }
    } catch { imgPayload = { url: src }; }
  }

  const prompt = document.getElementById('img-edit-ai-prompt')?.value || '';
  const body = { action, prompt, ...imgPayload };

  try {
    const res = await fetch('/api/images/edit', { method:'POST',
      headers:{'Content-Type':'application/json'}, body: JSON.stringify(body) });
    const data = await res.json();
    if (data.success) {
      const newSrc = data.type === 'url' ? data.url : data.image;
      imgEl.src = newSrc;
      _lightboxCurrentUrl = newSrc;
      if (dl) { dl.href = newSrc; dl.download = `nexus-edit-${action}.png`; }
      if (status) { status.textContent = `✓ ${action} applied`; status.style.color = '#a78bfa'; }
    } else {
      if (status) { status.textContent = `Error: ${data.error}`; status.style.color = '#f87171'; }
    }
  } catch (e) {
...
// ══════════════════════════════════════════════════════
// iOS MOBILE-FIRST — keyboard handling + scroll button
// ══════════════════════════════════════════════════════

(function initMobileIOS() {
  const isMobile = () => window.innerWidth <= 768;

  // ── visualViewport keyboard handler ──
  // When iOS keyboard opens, visualViewport.height shrinks.
  // We translate #input-area up by the keyboard height to keep it visible.
  function _handleViewportResize() {
    ...
    const keyboardH = Math.max(0, offsetBottom);

    if (keyboardH > 50) {
      // Keyboard is open — lift input area above keyboard
      inputArea.style.transform  = `translateY(-${keyboardH}px)`;
      inputArea.style.transition = 'transform 0.1s ease-out';
      if (chatArea) {
        chatArea.style.bottom = `calc(var(--input-bar-h, 68px) + ${keyboardH}px)`;
      ...
      inputArea.style.transition = 'transform 0.12s ease-out';
      if (chatArea) chatArea.style.bottom = '';
    }
  }

  function _handleViewportScroll() {
    // Re-apply on scroll to handle Safari address bar hide/show
    _handleViewportResize();
  }

  if (window.visualViewport) {
    window.visualViewport.addEventListener('resize', _handleViewportResize, { passive: true });
    window.visualViewport.addEventListener('scroll', _handleViewportScroll, { passive: true });
  }

  // ── Prevent body scroll bounce on iOS ──
  // Only allow scroll inside designated scrollable areas
  document.body.addEventListener('touchmove', (e) => {
    const scrollable = e.target.closest(
      '#chat-area, #history-list, #live-console-output, #terminal-output, ' +
      '#kali-live-output, #cpd-body, .gallery-scroll, #img-lightbox, ' +
      '.proc-monitor-body, #gallery-grid'
    );
    if (!scrollable) e.preventDefault();
  }, { passive: false });

  // ── Focus input: scroll chat to bottom when keyboard opens ──
  ...
  let _unreadCount = 0;
  const _origScrollToBottom = window.scrollToBottom;
  ...
  function _incrementUnread() {
    if (!document.getElementById('scroll-bottom-btn')?.classList.contains('visible')) return;
    _unreadCount++;
    const btn   = document.getElementById('scroll-bottom-btn');
    const badge = document.getElementById('scroll-unread-badge');
    if (btn)   btn.classList.add('has-unread');
    if (badge) badge.textContent = _unreadCount > 9 ? '9+' : String(_unreadCount);
  }

  // Patch scrollToBottom to reset unread
  window._mobileResetUnread    = _resetUnread;
  window._mobileIncrementUnread = _incrementUnread;

  // Intercept new message rendering to count unread
  const _origAppendMsg = window.appendMessage;
  if (typeof _origAppendMsg === 'function') {
    window.appendMessage = function(...args) {
      const result = _origAppendMsg.apply(this, args);
      if (typeof _userScrolledUp !== 'undefined' && _userScrolledUp) _incrementUnread();
      return result;
    };
  }
  ...
  // ── Swipe-to-open drawers (left edge → history, right edge → gallery) ──
  ...
  document.addEventListener('touchend', (e) => {
    ...
    // Left edge swipe right → open history
    if (_touchStartX < 24 && dx > 50) {
      if (typeof openHistory === 'function') openHistory();
    }
    // Right edge swipe left → open gallery
    if (_touchStartX > window.innerWidth - 24 && dx < -50) {
      if (typeof toggleImageGallery === 'function') toggleImageGallery();
    }
  }, { passive: true });
